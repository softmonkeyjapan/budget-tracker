...
test('a child category without its own color inherits the parent resolved color and icon', function () {
    $user = User::factory()->create();
    $root = Category::factory()->for($user)->create(['color' => '#2F80ED', 'icon' => '🍔']);
    Category::factory()->child($root)->create(['color' => null, 'icon' => null]);

    $response = $this->actingAs($user)->get('/categories');

    $response->assertInertia(fn (Assert $page) => $page
        ->where('categories.0.children.0.resolved_color', '#2F80ED')
        ->where('categories.0.children.0.resolved_icon', '🍔')
        ->where('categories.0.children.0.color_inherited', true)
        ->where('categories.0.children.0.icon_inherited', true)
    );
});

test('a child category with its own color keeps it', function () {
    $user = User::factory()->create();
    $root = Category::factory()->for($user)->create(['color' => '#2F80ED']);
    Category::factory()->child($root)->create(['color' => '#FF5B62']);

    $response = $this->actingAs($user)->get('/categories');

    $response->assertInertia(fn (Assert $page) => $page
        ->where('categories.0.children.0.resolved_color', '#FF5B62')
        ->where('categories.0.children.0.color_inherited', false)
    );
});
